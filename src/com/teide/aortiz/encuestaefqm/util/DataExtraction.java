...
public class DataExtraction {
    ...
    public static final String CADENA_CORTE = ",";
    ...
    public void analizaResponsables() throws Exception {
        ...
        String[] campos = cadena.split(CADENA_CORTE);
        int i = NUM_INICIAL;
        for (; i < campos.length; i++) {
            ...
            if (campos[i].contains(">")) {
                if (pos!=-1) {                   
                    if (!nombresAnalizados[pos].contains(((campos[i].split(">"))[1]).replace("\"", ""))) 
                        //Ahora desde la versión Moodle 3.6 nos pone el nombre entre dobles comillas así que lo quitamos
                        nombresAnalizados[pos].add(((campos[i].split(">"))[1]).replace("\"", ""));
                }
            }
    ...
    private ArrayList<String>[] analizarRespuesta (String respuesta) {
        ...
        String[] campos = respuesta.split(CADENA_CORTE);
        int pos = NUM_INICIAL;
